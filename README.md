...
后台服务开关会和配置一起保存，跨 App 重启和 macOS 重新登录保持。关闭开关会停止并移除 LaunchAgent，不会在下次登录时自行恢复。
...
收件箱与整理计划都消费原生 Agent 的同一份结构化文件评估结果；界面显示的状态、原因、目标和等待时间与实际执行使用同一套判断。v3 RC 仍需在真实 macOS 用户环境中验证 LaunchAgent 生命周期、TCC 隐私权限和不同文件系统行为。
...
- 配置版本会在原生 App 保存时规范化为当前版本 10；后台服务期望状态保存在 `automation_enabled`，旧版配置会根据既有 LaunchAgent 状态迁移，不改变整理模式；旧版扩展名字段会补齐 CSV/TSV 等兼容项。
...
建议先使用“文件名测试”和“整理计划”检查规则，再启用后台服务。默认配置是审阅模式且后台服务关闭，默认不额外延迟文件；下载未完成保护仍由临时后缀和稳定时间检测负责。
...
├── mac-app/
│   ├── Info.plist
│   ├── Sources/                 # SwiftUI App、共享评估模型、服务和原生 Agent
...
本 RC 未声称已完成 Developer ID 签名或 notarization；正式发布前仍需在真实用户环境完成验证。
