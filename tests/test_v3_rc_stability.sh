...
test "$(plutil -extract schema_version raw "$SCAN_ROOT/scan.json")" = "1"
grep -q '"watch_folder"' "$SCAN_ROOT/scan.json"
grep -q '"status":"automatic_pending"\|"status":"awaiting_confirmation"' "$SCAN_ROOT/scan.json"
...
grep -q '"can_select":false' "$SCAN_ROOT/scan.json"
